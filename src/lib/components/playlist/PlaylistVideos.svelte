...
<div class="flex flex-col gap-4">
	{#each playlist.videos as video, index}
		<div class="grid grid-cols-[50px_1fr] items-center gap-4">
			<span class="text-center dark:text-neutral-400">{index + 1}</span>
			<PlaylistVideoCard videoObject={video} playlistId={playlist.playlistId} {index} />
		</div>
	{/each}
</div>
